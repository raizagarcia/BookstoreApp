...
export function StoreItem({ id, name, author, price, imgUrl}: StoreItemProps){
    const {getItemQuantity, increaseCartQuantity, decreseCartQuantity, removeFromCart} = useShoppingCart()
    ...
    return <Card className="h 100">
        ...
        <Card.Body className="d-flex flex-column">
            ...
            justify-content-space-between align-items-baseline mb-4">
                ...
                <span className="ms-2 fs-5">{author}</span>
                <span className="ms-2 text-muted">{formatCurrency(price)}</span>
            </Card.Title>
            
            <div className="mt-auto">
                {quantity === 0 ? (
                    <Button variant="success" onClick={() => increaseCartQuantity(id)}>Add to cart</Button>
                ...
                flex-column" style={{ gap: ".5rem"}}>
                        ...
                        style={{ gap: ".5rem"}}>
                            <Button  variant="success" onClick={() => decreseCartQuantity(id)}>-</Button>
                            <div>
